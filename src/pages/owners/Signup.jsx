...
function OwnerSignup() {
  ...
  return (
    <div className="flex min-h-[calc(100vh-5rem)] items-center justify-center bg-slate-950 px-4 py-8 text-slate-50">
      <motion.div
        initial={{ opacity: 0, y: 14 }}
        animate={{ opacity: 1, y: 0 }}
        className="w-full max-w-md rounded-3xl border border-slate-800/80 bg-slate-900/80 p-6 shadow-2xl shadow-emerald-500/20 backdrop-blur-xl"
      >
        <div className="mb-5 text-center">
          <p className="mb-2 text-[11px] font-medium uppercase tracking-[0.2em] text-emerald-400">
            Join JOHNBOOKS
          </p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          ...
          <div>
            ...
            <Textarea
              ...
              className="rounded-2xl border-slate-700 bg-slate-950/80 text-xs"
              {...register("bio")}
            />
          </div>

          {/* Info text */}
          <div className="mt-1 rounded-2xl border border-emerald-500/30 bg-emerald-500/5 px-3 py-2 text-[11px] text-emerald-200">
          ...
          <Button
            ...
            className="mt-3 h-9 w-full rounded-2xl bg-gradient-to-r from-emerald-500 to-cyan-500 text-xs font-semibold text-slate-950 shadow-lg shadow-emerald-500/40 hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-60"
          >
